...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE profesores SET nombre = ?, apellido = ?, especialidad = ?, telefono = ?, correo = ? WHERE id = ?");
        $stmt->execute([$nombre, $apellido, $especialidad, $telefono, $correo, $id]);
        header("Location: index.php");
        exit;
    }
}
